...
world_shape_pacific <- world_shp %>% st_shift_longitude() %>% st_transform(crs = mollweide)

# world_shp_pacific <- world_shp %>% st_difference(polygon) %>% st_transform(crs = mollweide_pacific)
bbox_pacific <- bbox_shp %>% st_shift_longitude() %>% st_transform(crs = mollweide)
...
(species_landings <- ggplot() +
  ...
  scale_fill_manual(values = rev(colorRampPalette(brewer.pal(n=9, name = "YlGnBu"))(11)))+
...
ggsave(here("explore/cottrell_explore/temporal_spatial_landings.jpg"), device = "jpg", dpi=600, height = 6, width = 17, units = "cm")
```
