...
ecosystem_vitality_2020 <- readRDS(file=here("data", "tidy_data", "ecosystem_vitality.rds")) %>% dplyr::select(-country) %>% 
  rename(ecosystem_vitality = ECO.new)

epi_2020 <- readRDS(file=here("data", "tidy_data", "environmental_performance_index.rds")) %>% dplyr::select(-country) %>% 
  rename(epi = EPI.new)

slavery_index <- readRDS(file = here("data", "tidy_data", "gsi.rds")) |> select(-country)

eiu_illicit_trade <- readRDS(here("data/tidy_data/eiu-illicit-trade.rds")) |> select(-rank, -country_name)
...
(aus_map <- ggplot()+
  ...
  theme_void()+
   annotate("text", x=135, y=-24, label= round(eiu_illicit_trade$epi_score[eiu_illicit_trade$country_name=="Australia"], digits = 2), size=2)
)
```
...
```{r}
china_illicit_trade_value <- eiu_illicit_trade |> filter(country_name == "China") |> pull(illicit_score)

all_metrics <- import_totals |> 
  left_join(epi_2020, by="iso3c") |> 
  left_join(slavery_index, by="iso3c") |>  
  left_join(eiu_illicit_trade, by="iso3c") |> 
  mutate(illicit_score = case_when(iso3c == "TWN" ~ china_illicit_trade_value, 
         TRUE ~ illicit_score)) |> 
...
ggplot()+
  geom_hline(yintercept = epi_2020 %>% filter(iso3c =="AUS") %>% pull(epi), linetype=2, colour="grey70")+
  geom_vline(xintercept = slavery_index %>% filter(iso3c =="AUS") |> pull(gsi_vuln), linetype=2, colour="grey70")+
  geom_point(data =  all_metrics, 
              aes(x=gsi_vuln, y=epi, colour=illicit_score, size=prop_le))+
  geom_text_repel(data = all_metrics %>% slice(1:20), 
                  mapping= aes(x=gsi_vuln, y=epi, label=partner_country_name), 
  ...
  annotation_custom(ggplotGrob(aus_map), xmin = 96, xmax = 104, ymin = 56, ymax=65)+

  labs(x="Vulnerability to slavery", y="Environmental Performance Index", colour="Illicit trade\nenvironment", size="Proportion of seafood imports")#+
