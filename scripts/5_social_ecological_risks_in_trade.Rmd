...
ecosystem_viability_2020 <- readRDS(file=here("data", "tidy_data", "ecosystem_viability.rds")) %>% select(-country) %>% 
...
(aus_map <- ggplot()+
  geom_sf(data= sf::st_crop(x=aus_shp, y=c(xmin = 112.9082, xmax = 158.9589, ymin=-45, ymax=-10.05176)), fill=rev(brewer.pal(n=9, name = "PuRd"))[8], colour=NA)+
  theme_void()+
...
ggplot()+
  ...
  scale_color_gradientn(colours = rev(brewer.pal(n=9, name = "PuRd")))+
