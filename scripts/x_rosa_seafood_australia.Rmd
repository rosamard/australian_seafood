...
(aqu_aus_2 <- aqu_aus %>% 
    select(- c("country", "unit")) %>% 
    filter(!isscaap_group %in% c("Oysters", "Crabs, sea-spiders", "Mussels", "Scallops, pectens", "Miscellaneous aquatic invertebrates", "Miscellaneous marine molluscs", "Tunas, bonitos, billfishes", "Abalones, winkles, conchs"))) ## check tuna FCR apparently super  super high.
...
prod_FCR <- aqu_prod_eFCR %>% 
  group_by(Taxa, year) %>% 
  summarise(production = sum(value), eFCR, species, Taxa) %>% 
  drop_na(Taxa) %>% 
  ungroup() %>% 
  group_by(year) %>% 
...
ggplot(prod_FCR) + 
  geom_line( aes(x = year, y = eFCR, color= Taxa)) + 
  scale_fill_manual(values =  palette) +                     #brewer.pal(n=2, "Blues"))+
