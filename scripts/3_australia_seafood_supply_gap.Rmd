...
#Calculate the changing proportion of reexports, and non-food uses

flow_summary <- 
  readRDS(file = here("data", "tidy_data", "seafood_trade_volume_live_equivalent_1976_2019.rds")) |> 
  group_by(year, trade_flow_name) |> 
  summarise(value = sum(value_LE_med_cf, na.rm = TRUE)) |> 
  pivot_wider(names_from = trade_flow_name, values_from = value)
...
adj_fbs_aus <- 
  ...
  left_join(flow_summary, by = "year") |> 
  select(-c(Exports, Imports)) |> 
  mutate(Reexports = if_else(is.na(Reexports), true = 0, false = Reexports)) |> 
  mutate(`Domestic production` = Production-`Food exports`-(`Non-food uses`*0)+(`Stock variations`*0.5), #adj production by half of non food use and stock variations
         `Imports` = `Food imports`- Reexports - (`Non-food uses`*1)+(`Stock variations`*0.5)) %>% #adj imports by half of non food uses and stock variations
